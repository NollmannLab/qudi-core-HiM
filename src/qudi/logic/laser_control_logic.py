...
class LaserControlLogic(LogicBase):
    ...
    def on_activate(self):
        """ Initialisation performed during activation of the module. Create a dictionary
        """
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    @QtCore.Slot(int, float)  # should the decorator be removed when this method is called in a task ???
    def set_laser_line_intensity(self, wavelentgh, value):
        """Update the intensity associated to the selected laser - this method does not
        necessarily
        """
        if self.laser_dict[wavelentgh]["allowed"]:
            self.laser_dict[wavelentgh]["intensity"] = value
            if self._enabled:
                self._laser.apply_line_intensity(wavelentgh, value)
            else:
                self._laser.update_line_intensity(wavelentgh, value)
        else:
            self.log.warning(f"laser line {wavelentgh} nm is not allowed in the current configuration. ")
    ...
    def set_laser_enabled(self) -> None:
        ...
        self._laser.enable_all_lines()

    @QtCore.Slot()
    def set_laser_disabled(self) -> None:
    ...
    def reset_laser_intensities(self):
        ...
        """
        for wavelength in self.laser_dict:

            if not self.laser_dict[wavelength]["allowed"]:
                continue

            self.laser_dict[wavelength]["intensity"] = 0

            if self._enabled:
                self._laser.apply_line_intensity(wavelength, 0)
            else:
                self._laser.update_line_intensity(wavelength, 0)

        self.sigIntensityChanged.emit()

    def set_external_trigger(self, enabled: bool) -> None:
        """Enable or disable external trigger control."""
        self._laser.set_ttl(enabled)
