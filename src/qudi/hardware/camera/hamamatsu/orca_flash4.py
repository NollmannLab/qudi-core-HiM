    ...
    def on_activate(self):
        ...
        try:
            if Dcamapi.init():
                ...
                if n==0:
                    self.log.error('Hamamatsu camera not connected. Check if device is switched on and HCImageLive is '
                                   'closed.')
                elif n==1:
                    self._camera = Dcam(0)
                    output = '#{}: '.format(0)
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    def set_image(self, hbin, vbin, hstart, hend, vstart, vend):
        ...
        try:
            ...
            vsize = vend - vstart
            hsize = hend - hstart
            self._camera.setPropertyValue('subarray_hpos', hstart)
            self._camera.setPropertyValue('subarray_vpos', vstart)
            self._camera.setPropertyValue('subarray_hsize', hsize)
            self._camera.setPropertyValue('subarray_vsize', vsize)
            self._camera.setSubArrayMode()
    ...
    def get_progress(self):
        ...
        """
        return self._camera.check_frame_number()
...
# Methods for displaying images on the GUI -----------------------------------------------------------------------------
    def start_single_acquisition(self):
        ...
        """
        try:
            self._camera.setACQMode('fixed_length', 1)
    ...
    def start_live_acquisition(self):
        ...
        """
        try:
            self._camera.setACQMode('run_till_abort')
            self._camera.startAcquisition()
            return True
        except Exception:
            return False
...
# Methods for saving image data ----------------------------------------------------------------------------------------
    def start_movie_acquisition(self, n_frames):
        ...
        try:
            self._camera.setACQMode('fixed_length', n_frames)
            self._camera.startAcquisition()
            return True
        except Exception:
            return False
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    def get_most_recent_image(self):
        ...
        """
        [frame, dim] = self._camera.getMostRecentFrame()  # frame is HCamData object, dim is a list [image_width, image_height]
    ...
    def get_acquired_data(self):
        ...
        [frames,
         dim] = self._camera.getFrames()  # frames is a list of HCamData objects, dim is a list [image_width, image_height]
        ...
        if acq_mode == 'run_till_abort':
            ...
            while not frames and counter < 10:  # check if frames list is empty; 10 tries to get the data
                [frames, dim] = self._camera.getFrames()  # try again to get the data
                counter += 1
                sleep(0.005)
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    def _set_trigger_polarity(self, polarity):
        ...
        @return int check_val: ok: 0, not ok: -1
        """
        check_val = self._camera.setPropertyValue('trigger_polarity', polarity)  # returns a float corresponding to the polarity (1.0: negative, 2.0: positive) or bool False if not set
        if isinstance(check_val, float):
            return 0
        else:
            return -1
    ...
    def _configure_output_trigger(self, channel, output_trigger_kind, output_trigger_polarity):
        ...
        """
        trigger_kind = self._camera.setPropertyValue(f'output_trigger_kind[{channel}]', output_trigger_kind)
        print(trigger_kind)
