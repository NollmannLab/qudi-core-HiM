...
class ThorlabsMotorizedFilterWheel(FilterWheelInterface):
    ...
    def on_activate(self):
        ...
        if len(self._filternames) != self._num_filters or len(self._positions) != self._num_filters or len(
                self._allowed_lasers) != self._num_filters:
    ...
    def set_position(self, target_position):
        ...
        if target_position < self._num_filters + 1:
            ...
            self.log.error('Can not go to filter {0}. Filterwheel has only {1} positions'.format(target_position,
                                                                                                 self._num_filters))
            err = -1
        return err
