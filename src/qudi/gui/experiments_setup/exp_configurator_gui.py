...
This module displays the experiment form and synchronizes its widgets with ``ExpConfigLogic``. The form
is data-driven: an experiment-definition YAML file selects which existing fields are visible, while the
logic supplies defaults, validation, loading, and saving.

Configuration files must not be confused:

* The global Qudi configuration activates this GUI and the configurator logic, connects the two modules,
sets setup-specific paths, and tells the logic which experiment-definition filenames are enabled.

* An experiment-definition YAML file describes one experiment type: its display name, description, output
filename, fields, defaults, visibility, and required values.

* A generated experiment configuration is the user-filled YAML file saved by this GUI and later read by an
experiment task.

Adding a new experiment
-----------------------
For an experiment that uses only existing GUI fields, this module does not need
new experiment-specific code:

1. Create the experiment-definition YAML file in the directory selected by the logic option ``experiment_definitions_directory``.
2. Add that filename to the logic module's ``experiments`` list in the global Qudi configuration.
3. Reactivate the modules or restart Qudi, select the experiment, and verify the displayed fields and saved output.

Modify this GUI only when the experiment introduces a field or editor that does not already exist. In that case:

1. Add the required widget or widgets to ``ui_exp_configurator.ui``.
2. Register the field in ``FIELD_SECTIONS`` and ``FIELD_WIDGETS``.
3. Connect the widget signal to a logic update method in ``on_activate``.
4. Populate the widget from ``config_dict`` in ``update_entries`` and include it in ``_configuration_input_widgets``
so programmatic updates are signal-safe.
5. Extend the imaging-model handling only when the new experiment needs a new imaging-sequence representation.

A typical global Qudi configuration contains separate entries for the logic and
GUI modules::
    ...
    experiments_configurator_gui:
      module.Class: experiments_setup.exp_configurator_gui.ExpConfiguratorGUI
      connect:
        experiment_logic: experiments_configurator_logic
      options:
        default_path: /path/to/qudi_files

The ``experiments`` list above contains definition filenames. The human-readable experiment names shown in the
combo box come from the ``experiment`` key inside those files.

Author: F. Barho. Adapted to qudi-core by JB Fiche with chatGPT.
Reformatted 2026-07-31.
...
class ExpConfiguratorWindow(QtWidgets.QMainWindow):
    """Main window loaded from the Qt Designer ``.ui`` file."""
    def __init__(self, ui_filename):
        """Load and show the configurator window.

        Args:
            ui_filename: Name of the ``.ui`` file located beside this Python module.
        """
...
class ExpConfiguratorGUI(GuiBase):
    """GUI controller for creating and editing experiment task configurations.

    The class does not hardcode a branch for every experiment. Instead,
    ``FIELD_SECTIONS`` maps each supported configuration key to a form section,
    ``FIELD_WIDGETS`` maps the key to existing Qt widgets, and
    ``apply_experiment_definition`` uses a loaded definition to decide what is
    visible. ``ExpConfigLogic`` owns the actual configuration values and YAML I/O.
    """
    ...
    def __init__(self, config, **kwargs):
        # load connection
        """Initialize module references; the Qt window is created during activation."""
        super().__init__(config=config, **kwargs)
        self._exp_logic = None
        self._mw = None

    def on_activate(self):
        """Create the window and connect it to the configurator logic.

        The experiment selector is populated from definitions already loaded by the
        logic. Setup-dependent combo boxes and the default imaging model are initialized,
        then toolbar actions, form widgets, and logic signals are connected.
        """
    ...
    def on_deactivate(self):
        """Close the configurator window when the GUI module is deactivated."""
        self._mw.close()

    def show(self):
        """Show, raise, and activate the configurator window."""
...
# ----------------------------------------------------------------------------------------------------------------------
    def _set_named_widgets_visible(self, widget_names, visible: bool) -> None:
        """Set visibility for widgets referenced by Qt object name.

        Missing object names are logged instead of raising, which makes definition and
        UI mismatches easier to diagnose.
        """
    ...
    def _hide_experiment_form(self) -> None:
        """Hide every configurable section and disable both save actions.

        This resets presentation state before another experiment definition is applied,
        preventing widgets from the previous experiment from remaining visible.
        """
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    def apply_experiment_definition(self, definition: dict) -> None:
        """Configure the form from one parsed experiment definition.

        Fields present in the definition are visible by default unless their
        ``visible`` value is false. Required sections are inferred from
        ``FIELD_SECTIONS``; optional explicit ``sections`` metadata may add sections.
        After section containers are shown, individual fields are filtered through
        ``FIELD_WIDGETS``. The imaging editor, experiment description, and save actions
        are then updated.

        Args:
            definition: Parsed experiment-definition mapping selected by the user.

        Raises:
            TypeError: If the definition or its ``fields`` entry has the wrong type.
            ValueError: If the definition has no experiment name.
        """
    ...
    def _apply_imaging_sequence_definition(self, fields: dict) -> None:
        """Select and configure the imaging-sequence editor.

        The standard model is used unless the field metadata requests
        ``timelapse_ramm`` or ``timelapse_palm``. Setup lasers are restored each time
        and any ``additional_sources`` declared by the experiment are appended.

        Args:
            fields: The selected definition's field mapping.
        """
    ...
    def start_new_experiment_config(self, experiment: str) -> None:
        """Start a fresh form for the experiment selected in the combo box.

        The logic creates a configuration from definition defaults, the GUI applies the
        field visibility rules, and widgets are populated with the new values. Selecting
        the placeholder hides the form.
        """
    ...
    def set_visibility_general_settings(self, visible):
        """Show or hide the general section's base widgets.

        Experiment-specific fields such as DAPI, RNA, and email are hidden here by
        default and shown later through ``FIELD_WIDGETS`` when declared by the active
        definition.
        """
    ...
    def set_visibility_camera_settings(self, visible):
        """Show or hide the camera-settings section and its widgets."""
    ...
    def set_visibility_filter_settings(self, visible):
        """Show or hide the filter-wheel section and its widgets."""
    ...
    def set_visibility_imaging_settings(self, visible):
        """Show or hide the imaging-sequence editor and controls."""
    ...
    def set_visibility_save_settings(self, visible):
        """Show or hide local, network, transfer, and file-format controls.

        ZEN-specific save controls are hidden here by default and are shown later only
        when their field is declared by the active definition.
        """
    ...
    def set_visibility_scan_settings(self, visible):
        """Show or hide z-stack and focal-plane controls."""
    ...
    def set_visibility_documents_settings(self, visible):
        """Show or hide ROI and injection-document controls.

        Legacy DAPI-path widgets are hidden by default and may be re-enabled through the
        field mapping when a definition declares ``dapi_path``.
        """
    ...
    def set_visibility_prebleaching_settings(self, visible):
        """Show or hide the photobleaching illumination controls."""
    ...
    def set_visibility_timelapse_settings(self, visible):
        """Show or hide timelapse iteration and calibration controls."""
    ...
    def set_visibility_ZEN_security_settings(self, visible):
        """Show or hide ZEN autofocus reference and threshold controls."""
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    def save_config_clicked(self):
        """Save using the definition's default output filename.

        The destination directory is ``qudi_task_config_files`` below the GUI's
        ``default_path`` option.
        """
    ...
    def save_config_copy_clicked(self):
        """Prompt for a filename and save a copy of the active configuration."""
        path = os.path.join(self.default_location, 'qudi_task_config_files')
        experiment = self._mw.select_experiment_ComboBox.currentText()
    ...
    def load_config_clicked(self):
        """Prompt for a generated experiment configuration and ask the logic to load it."""
    ...
    def clear_all_clicked(self):
        """Reset the selected experiment to the defaults in its definition file."""
        experiment = (
            self._mw
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    def add_entry_clicked(self):
        """Add the current imaging controls to the active sequence model.

        For timelapse models, per-entry z-stack values and the PALM filter position are
        included in the emitted signal. Standard models use only light source and
        intensity.
        """
    ...
    def delete_entry_clicked(self):
        """Request deletion of the currently selected imaging-sequence row."""
    ...
    def load_dapi_path_clicked(self):
        """Choose the directory containing associated DAPI data."""
    ...
    def load_ref_images_path_clicked(self):
        """Choose the directory containing ZEN autofocus reference images."""
        this_dir = QtWidgets.QFileDialog.getExistingDirectory(self._mw,
    ...
    def load_zen_saving_path_clicked(self):
        """Choose the directory in which ZEN should save experiment data."""
        this_dir = QtWidgets.QFileDialog.getExistingDirectory(self._mw,
    ...
    def load_axial_calibration_path_clicked(self):
        """Choose an axial-calibration file and place its path in the form."""
        # data_directory = os.path.join(self.default_location, 'qudi_roi_lists')
        this_file = QtWidgets.QFileDialog.getOpenFileName(self._mw,
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    def update_entries(self):
        """Populate all configuration widgets from ``logic.config_dict``.

        Widget signals are blocked during assignment so programmatic updates do not
        feed back into the logic as user edits. Imaging models are also told to refresh
        their displayed rows.
        """
    ...
    def update_listview(self):
        """Refresh imaging-sequence rows and clear any stale row selection."""
        self._exp_logic.img_sequence_model.layoutChanged.emit()
        self._exp_logic.img_sequence_model_timelapse_ramm.layoutChanged.emit()
        self._exp_logic.img_sequence_model_timelapse_palm.layoutChanged.emit()
    ...
    def display_loaded_config(self):
        """Render a configuration loaded by the logic.

        The experiment combo box is changed without emitting its selection signal, the
        matching definition is applied, and saved values are copied into the widgets.
        """
    ...
    def update_list_model(self, model):
        """Display the imaging model selected by the logic.

        Args:
            model: ``0`` for the standard model, ``1`` for RAMM timelapse, or ``2`` for
                PALM timelapse.
        """
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    def _configuration_input_widgets(self):
        """Return widgets whose signals must be blocked during GUI refreshes."""
        return (
